...
test.describe('Wallet Full Cycle', () => {
  ...
  test('دورة كاملة: إيداع → موافقة → سحب → رفض', async ({ page, request }) => {
    ...
    // 2. Open wallet page
    await page.goto(`${BASE}/ar/admin/wallet/requests`, { waitUntil: 'networkidle' });
    expect(page.url()).toContain('admin/wallet');
  ...
  test('المشتري يقدر يفتح المحفظة', async ({ page }) => {
    ...
    const response = await page.goto(`${BASE}/ar/dashboard/wallet`, { waitUntil: 'networkidle' });
